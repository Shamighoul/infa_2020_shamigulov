...
class Enemy():
    ...
    def boom(self, t_step=1.):
        ...
        for i in range(2):
            vel[i] = [-10*np.cos(self.angle[i]),-10* np.sin(self.angle[i]) ]
        return Bomb(list(self.coord), list(vel[0]), list(vel[1]))
        
class Target():
    '''заждает параметры начальной мишени'''
    def __init__(self):
        self.x = randint(300, 700)
        self.y = randint(51, 450)
        self.r = randint(15, 50)
...
class Manager():
    ...
    def helth(self, screen):
        '''рисует очки жизни'''
        for i in range(hp):
            pg.draw.polygon(screen, RED, [[784,35+i*15],[792,45+i*15],[796,30+i*15],[792,35+i*15],[788,30+i*15],[782,35+i*15]] )
    ...
    def handle_events(self, events):
        ...
        for event in events:
            ...
            if event.type == pg.QUIT or balls>=100 or hp<=0:
                done = True
